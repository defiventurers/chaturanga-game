...
function closeSocket(ws) {
  try { ws.end(Buffer.from([0x88, 0x00])); } catch (_error) {}
}
...
function attachFrameParser(ws, onText) {
  ...
  ws.on("data", (chunk) => {
    ...
    while (buffer.length >= 2) {
      ...
      if (opcode === 0x9) {
        try { ws.write(Buffer.from([0x8a, 0x00])); } catch (_error) {}
        continue;
      }
...
function handleJoinRoom(ws, payload, requestId) {
  try {
    ...
    if (!existing && getRoomPlayers(room).length >= room.playerCount) throw new Error("Room is full.");

    const preferredTeam = getFirstAvailableTeam(room, "blue", clientId) || getFirstAvailableTeam(room, null, clientId);
    if (!existing && !preferredTeam) throw new Error("No teams are available in this room.");
...
server.on("upgrade", (req, socket) => {
  ...
  socket.on("close", () => {
    ...
    if (peers && peers.get(socket.clientId) === socket) peers.delete(socket.clientId);
  });
